		...
		if("edit")
			if(computer && can_run(user, 1))
				...
				if(href_list["name"])
					...
					var/sug_fingerprint_hash = sanitize(input("Enter fingerprint hash.", "Fingerprint hash", id_card.fingerprint_hash), 33)
					if(hash_check.Find(sug_fingerprint_hash))
						id_card.fingerprint_hash = sug_fingerprint_hash
				else if(href_list["dna_hash"])
					var/sug_dna_hash = sanitize(input("Enter DNA hash.", "DNA hash", id_card.dna_hash), 33)
